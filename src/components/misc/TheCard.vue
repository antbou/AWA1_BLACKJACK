<template>
  <div
    :class="[
      'bg-black/10 dark:bg-black/70 flex flex-col rounded-lg p-4',
      cardHover ? 'hover:bg-black/70 hover:dark:bg-white/20' : '',
      cardTransition
  ...
  >
    ...
    <h1
      v-if="hasHeaderSlot"
      class="card-header font-roboto text-3xl md:text-4xl my-2"
    >
      <slot name="header" />
    </h1>
...
import AppDivider from "@/components/misc/TheDivider.vue";
...
const imgSrc = computed(() => {
  return typeof props.img === "string" ? props.img : "";
});
</script>
