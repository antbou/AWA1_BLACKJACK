<template>
  ...
  >
    ...
    <div class="modal-container p-4 w-full max-w-2xl m-auto">
      <div
        class="modal-body relative rounded-lg shadow bg-neutral-100 dark:bg-neutral-800"
      >
        <!-- Modal header -->
        <div
          v-if="hasHeaderSlot"
          class="modal-header flex justify-between items-start p-5 rounded-t border-b dark:border-gray-600"
        >
          <h3
          ...
          <button
            type="button"
            class="bg-transparent hover:bg-black/10 dark:hover:bg-white/10 text-gray-400 hover:text-gray-900 dark:hover:text-white rounded-lg text-sm p-1.5 ml-auto inline-flex items-center"
            @click="closeModal"
          >
            <svg
              class="w-5 h-5"
        ...
        <div
          v-if="hasFooterSlot"
          class="modal-footer flex items-center p-6 space-x-2 rounded-b border-t border-gray-200 dark:border-gray-600"
        >
          <slot name="footer" />
        </div>
      </div>
